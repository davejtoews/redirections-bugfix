...
<body>
	<div class="rank-math-redrection-debug">
		<h1><?php esc_html_e( 'Redirection Debugger', 'redirections' ); ?></h1>
		<p>
			<?php esc_html_e( 'Redirecting from ', 'redirections' ); ?><code>/<?php echo $this->uri; ?></code>
			<?php esc_html_e( ' To ', 'redirections' ); ?><code><a href="<?php echo $this->redirect_to; ?>"><?php echo $this->redirect_to; ?></a></code>
		</p>
		...
		<div class="rank-math-redirection-loading-timer">
			...
			<span class="redirection-timer"><?php printf( esc_html__( 'Redirecting in %s seconds...', 'redirections' ), '<span id="redirection-timer-counter">5</span>' ); ?></span><br />
			<a href="#" class="button button-secondary" id="rank-math-cancel-redirection"><?php esc_html_e( 'Stop Redirection', 'redirections' ); ?></a>
		</div>
		<div class="rank-math-redirect-debug-continue" style="display: none;">
			<a href="<?php echo $this->redirect_to; ?>"><?php esc_html_e( 'Continue redirecting', 'redirections' ); ?></a>
		</div>

		<p>
			<?php $page_url = Helper::get_admin_url( 'redirections' ); ?>
			<?php if ( is_array( $this->matched ) ) : ?>
			<a target="_blank" href="<?php echo $page_url . '&action=edit&redirection=' . $this->matched['id']; ?>"><?php esc_html_e( 'Manage This Redirection', 'redirections' ); ?></a> or
			<?php endif; ?>
			<a target="_blank" href="<?php echo $page_url; ?>"><?php esc_html_e( 'Manage All Redirections', 'redirections' ); ?></a>
		</p>

		<p class="rank-math-redirection-debug-info">
			<?php echo wp_kses_post( __( '<strong>Note:</strong> This interstitial page is displayed only to administrators. Site visitors are redirected without delay.', 'redirections' ) ); ?>
		</p>
