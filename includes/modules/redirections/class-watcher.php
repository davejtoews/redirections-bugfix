...
class Watcher {
		...
		if ( 'publish_to_publish' === $transition && $this->has_permalink_changed( $before_permalink, $after_permalink ) ) {
			...
			Helper::add_notification(
				sprintf(
					...
					__( 'SEO Notice: you just changed the slug of a %1$s and Rank Math has automatically created a redirection. You can edit the redirection by <a href="%2$s">clicking here</a>.', 'redirections' ),
	...
	public function display_suggestion( $post_id ) {
		...
		if ( $this->can_display_suggestion( $post ) ) {
			...
			$message = sprintf( wp_kses_post( __( '<strong>SEO Notice:</strong> A previously published post has been moved to trash. You may redirect it <code>%2$s</code> to <a href="%1$s">new url</a>.', 'redirections' ) ), $admin_url, $url );
	...
	private function can_display_suggestion( $post ) {
		...
		$excluded_post_types = [ 'attachment', 'revision', 'nav_menu_item', 'custom_css', 'customize_changeset', 'user_request' ];
		return ! in_array( $post->post_type, apply_filters( 'rank_math_redirections/excluded_post_types', $excluded_post_types ) );
	}
