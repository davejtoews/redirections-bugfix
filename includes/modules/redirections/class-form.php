...
use RankMath_Redirections\Monitor\DB as Monitor_DB;
...
class Form {
	...
	public function display() {
		?>
		<h2><strong><?php echo ( $this->is_editing() ? esc_html__( 'Update', 'redirections' ) : esc_html__( 'Add', 'redirections' ) ) . ' ' . esc_html( get_admin_page_title() ); ?></strong></h2>
		...
		<form class="cmb-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			<footer class="form-footer rank-math-ui">
				<button type="button" class="button button-link-delete button-xlarge alignleft"><?php esc_html_e( 'Cancel', 'redirections' ); ?></button>
				<button type="submit" class="button button-primary button-xlarge"><?php echo $this->is_editing() ? esc_html__( 'Update Redirection', 'redirections' ) : esc_html__( 'Add Redirection', 'redirections' ); ?></button>
			</footer>
		</form>
	...
	public function register_form() {
		...
		$cmb->add_field([
			'id'      => 'sources',
			'type'    => 'group',
			'name'    => esc_html__( 'Source URLs', 'redirections' ),
			'options' => [
				'add_button'    => esc_html__( 'Add another', 'redirections' ),
				'remove_button' => esc_html__( 'Remove', 'redirections' ),
			],
		...
		$cmb->add_field([
			'id'   => 'url_to',
			'type' => 'text',
			'name' => esc_html__( 'Destination URL', 'redirections' ),
		]);
		...
		$cmb->add_field([
			'id'      => 'status',
			'type'    => 'radio_inline',
			'name'    => esc_html__( 'Status', 'redirections' ),
			'options' => [
				'active'   => esc_html__( 'Activate', 'redirections' ),
				'inactive' => esc_html__( 'Deactivate', 'redirections' ),
			],
			'default' => 'active',
		]);
	...
	public function save() {
		...
		if ( false === $redirection->save() ) {
			Helper::add_notification( __( 'Please add at least one valid source URL.', 'redirections' ), [ 'type' => 'error' ] );
